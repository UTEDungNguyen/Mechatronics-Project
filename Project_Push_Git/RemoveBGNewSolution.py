...
while True:
    #img = img_out.copy()
    img = cv2.imread(r'Result Remove Background\sample No.15_processed.jpg')
    # sharpened_image = sharpen_image_laplacian(img)
    # img = sharpened_image.copy()
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    thresh, output_otsuthresh = cv2.threshold(gray, 0, 255, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    ...
    getcoutours(border,img)
    # output = sub - output_adapthresh
    # Display the original and processed images
    cv2.imshow("Original Image", img)
    # cv2.imshow("Background Removed Image", img_out)
    
    # cv2.imshow("Gray image", output_dilate)
    # cv2.imshow("Thresh image", output_adapthresh )
    # cv2.imshow("sub", sub)
    # cv2.imshow("out image ", output)
    # cv2.imshow("output_otsuthresh", output_otsuthresh)
    # cv2.imshow("output_erosion", output_erosion)
    
    cv2.imshow("gray", gray)
    cv2.imshow("current", sub )
