...
# # Load picture using OpenCV, convert to grayscale and detect edges
# image_rgb = cv2.imread('images/images.jpg')
# image_rgb = cv2.cvtColor(image_rgb, cv2.COLOR_BGR2RGB)  # OpenCV uses BGR by default
...
# Display the original image and the result using OpenCV
cv2.imshow('Original picture', cv2.cvtColor(image_rgb, cv2.COLOR_RGB2BGR))
cv2.imshow('Edge (white) and result (red)', img_as_ubyte(edges))
cv2.waitKey(0)
