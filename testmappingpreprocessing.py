...
def classification(imageDurian):
    # Mở thư viện opencv và lấy "bitmap" từ camera để chuyển sang "Mat" xử lý
    # Chuyển ảnh xám
    ...
    max_area = 0
    # kiểm tra contours thu về
    for contour in contours:
        #Đếm pixel bằng cách khoanh vùng và chạy kiểm tra các pixel từ contour hiện tại
        x, y, w, h = cv2.boundingRect(contour)
        dem = np.sum(thresh[y:y+h, x:x+w] == 255)
        #Lấy giá trị đếm pixel lớn nhất và tính diện tích thật của contour đó
        if dem > max_area:
            max_area = dem
            # 1369 / 4260 chiều rộng của pixel (mm/pixel), 851 / 2810 chiều cao của pixel (mm/pixel)
...
def to_bitmap_image_two(imshow): # Define Camera 1
    ...
    length = x2 - x1
    height = y4 - y3
    #Vẽ hình chữ nhật bao quanh trai sau rieng với chiều dài và chiều rộng thu được (vẽ bằng đơn vị pixel)
    cv2.rectangle(thresh, (x1, y3), (x1 + length, y3 + height), (0, 0, 0), 4, 0)

    pil_image = Image.fromarray(frame)
    bitmap_image = ImageTk.PhotoImage(pil_image)
    # Trả về giá trị bimapimage
    return bitmap_image
...
def to_bitmap_image_two1(imshow): #Define camera 2
...
# Example usage:
# Assuming you have a Bitmap object called imshow
# bitmap_image = to_bitmap_image_two1(imshow)
...
def detect_contours(imshow):
    ...
    if len(contours) > 0:
        while contour_index >= 0:
            ...
            bounding_rect = cv2.boundingRect(contour) # Find bounding rectangle for each contour 
...
def dem_contours(imageDurian):
    ...
    if len(contours) > 0:
        #Chạy lần lượt từng contours bắt được từ ảnh
        while contour_index >= 0:
            contour = contours[contour_index]
